...
## 22. Phase 11A additions (fulfillment)

- **Per-supplier timeline, never a unified bar.** The customer order detail gains an **Order progress** card with ONE vertical stepper per supplier (Payment confirmed → Waiting for supplier → Accepted → Preparing → Ready for next stage). Each stage is text + icon + date (never colour-only); the current stage carries a **Current** chip and future stages are subdued (opacity) — the state is obvious without relying on colour.
- **Calm supplier lifecycle controls.** The supplier order row is state-driven: **Accept order** / **Decline** for a new paid order, then **Start preparing**, then **Mark ready** — buttons come only from the state machine (a hidden button is never the only guard). Accept opens an inline `role="group"` confirmation; nothing auto-accepts.
- **Accessible structured decline.** Decline opens a real `<fieldset>`/`<legend>` of reasons (radio inputs) + an optional **internal note** with an `aria-describedby` hint stating it's never shown to the customer. The customer sees only a short, safe reason label.
- **Honest fulfillment status.** `CreditCard`/`Clock`/`Check`/`Hammer`/`PackageCheck`/`Ban` chips pair every fulfillment state with an icon + text; the account order card shows a deterministic one-line summary ("2 of 3 suppliers accepted"). Suppliers see only **Paid / Awaiting** for payment — never a failure reason.
- **No fake automation.** Notifications are labelled recorded in **Demo Mode**, never "notified by WhatsApp"; "Ready for next stage" is never "Out for delivery". Three domains (order / payment / fulfillment) stay visually and structurally distinct.

## 23. Phase 11B additions (custom manufacturing + quality check)

- **A manufacturing workspace, not an ERP.** The supplier dashboard gains a **Manufacturing** tab: jobs grouped into calm stage sections (To start / In production / Quality check / Needs rework / Ready) with refined cards (order number, item, spec category, quantity, estimate, stage, last update) — premium furniture identity, generous spacing, `Factory`/`Hammer`/`ClipboardCheck`/`Wrench`/`PackageCheck` status chips (text + icon, never colour-only).
- **Progressive-disclosure job detail.** Opening a job reveals the immutable accepted specification (with an "accepted spec — not editable here" note), the accepted-quote terms, a production timeline, an optional milestone checklist (genuine, no fake %), and the QC panel/history — state-gated actions only, sourced from the state machine.
- **A real QC surface.** The quality check is a `<fieldset>` of 8 furniture criteria (pass requires all) with a structured issue form (category, `minor`/`major` severity, description). QC history shows numbered attempts (QC #1 failed → QC #2 passed) — a failed inspection is never hidden or overwritten.
- **Customer-safe manufacturing timeline.** The order-detail timeline extends for custom items (Manufacturing started → completed → Quality check → Ready for delivery); during rework it reads calmly ("Quality review in progress / additional finishing required") and never surfaces QC issues or internal notes. Ready-stock items keep the Phase 11A fulfillment timeline unchanged.
- **Four quiet domains.** Order · payment · fulfillment · manufacturing stay visually and structurally distinct; catalog orders never show a false manufacturing state.

## 24. Phase 12 additions (delivery + installation + tracking)

- **A tracking moment, not a courier ERP.** The customer order detail leads with a **Delivery & tracking** section: one calm vertical tracker per supplier with customer-friendly stages (Preparing your delivery → Delivery scheduled → On the way → Delivered → Installation → Completed) — text + icon + dates (never colour-only), current "Now" chip, future stages subdued. **No map, no fake GPS, no "driver 2.3 km away".**
- **A shared, accessible slot picker.** Demo delivery/installation windows (Morning 9–12 / Afternoon 12–16 / Evening 16–20) over the next 7 days as `<fieldset>`/radio chips; past dates are never offered; clearly Demo-labelled. Reused by customer delivery scheduling and supplier installation scheduling.
- **Supplier delivery workspace.** A **Delivery** dashboard tab: deliveries grouped Awaiting scheduling / Scheduled / Out for delivery / Delivered / Needs attention / Completed; refined cards (order number, area, slot, installation-included, status). Detail view carries the operational address snapshot (with a "captured at order time" note), timeline + attempts, and state-gated actions — assign **Demo Delivery Team**, out for delivery, report failure (structured reason the customer never sees), reschedule, delivered, installation schedule/start/complete, confirm handover.
- **Customer-friendly failure language.** A failed delivery reads "We couldn't complete this delivery. A new time is being arranged." — never a raw internal reason code; the reason stays supplier-side.
- **Five quiet domains.** Order · payment · fulfillment · manufacturing · delivery stay visually and structurally distinct; the account card shows a deterministic delivery summary ("2 of 3 deliveries completed").

## 25. Phase 13 additions (notifications + memory + follow-up)

- **A calm notification center, not a feed.** A header **bell** with a small unread badge (`aria-live` count) opens a `role="dialog"` dropdown grouped **Today / Earlier** — each item an icon + localized title/body + timestamp + unread dot + mark-read/dismiss + deep link. **Mark all as read** at the top; Escape / click-outside to close. High-priority items get a quiet danger-tinted icon (paired with text, never colour-only). Only high-signal events — no noisy state-machine spam.
- **Personal, transparent memory.** Account → **Design memory & preferences**: two `role="switch"` opt-in toggles, editable style/colour/material chips (labelled remove buttons), a budget-range editor, and clearly separated **Clear all** vs **Turn off** with `role="dialog" aria-modal` confirms. Honest copy ("saved on this browser only… we never store passwords or payment details") — never a false encryption claim, never creepy.
